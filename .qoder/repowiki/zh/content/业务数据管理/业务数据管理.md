...
CheckRequired --> |是且为空| RequiredError["返回必填错误"]
CheckRequired --> |否或非空| TypeCheck{"检查字段类型"}
TypeCheck --> NumberValidation["数值类型验证<br/>- 类型转换<br/>- 范围检查"]
TypeCheck --> StringValidation["字符串类型验证<br/>- 长度检查<br/>- 正则匹配<br/>- 枚举验证"]
TypeCheck --> BooleanValidation["布尔类型验证"]
TypeCheck --> DateValidation["日期类型验证<br/>- RFC3339格式"]
TypeCheck --> ArrayValidation["数组类型验证<br/>- 长度检查"]
TypeCheck --> ObjectValidation["对象类型验证"]
NumberValidation --> ValidationResult["返回验证结果"]
StringValidation --> ValidationResult
BooleanValidation --> ValidationResult
DateValidation --> ValidationResult
ArrayValidation --> ValidationResult
ObjectValidation --> ValidationResult
RequiredError --> ValidationResult
```
...
系统实现了JQL查询语言与MongoDB查询的无缝集成：

#### JQL到MongoDB过滤器转换

```mermaid
flowchart TD
JQLInput[JQL查询输入] --> ParseJQL[解析JQL表达式]
ParseJQL --> GenerateFilter[生成MongoDB过滤器]
GenerateFilter --> PrefixFields[添加custom_fields前缀]
PrefixFields --> SystemFields{"是否为系统字段?"}
SystemFields --> |是| KeepAsIs[保持原样]
SystemFields --> |否| AddPrefix[添加custom_fields.前缀]
KeepAsIs --> FinalFilter[最终过滤器]
AddPrefix --> FinalFilter
FinalFilter --> ExecuteQuery[执行MongoDB查询]
```

**图表来源**
- [tech.md:215-241](file://docs/modules/business-data/tech.md#L215-L241)

#### 字段名前缀转换规则

系统字段名（不添加前缀）：
- `_id`, `module`, `description`
- `created_at`, `updated_at`
- `created_by`, `updated_by`
- `data_path`, `file_path`, `custom_fields`

**章节来源**
- [tech.md:226-241](file://docs/modules/business-data/tech.md#L226-L241)

### CRUD操作API

系统提供了完整的CRUD操作API，支持所有核心功能：
...
**章节来源**
- [tech.md:244-263](file://docs/modules/business-data/tech.md#L244-L263)
