...
const scraperService = {
  ...
  // 重试刮削任务
  retryScrapeTask: async (taskId: string): Promise<ApiResponse> => {
    const response = await apiClient.post(`/api/scraper/tasks/${taskId}/retry`, {})
  ...
  // 创建刮削任务
  createScrapeTask: async (data: { module: string; description: string; custom_fields?: any }): Promise<ApiResponse> => {
    const response = await apiClient.post('/api/scraper/upload', {
      module: data.module,
      data_path: '/data/' + data.module,
      scraper_path: '/scrapers/' + data.module + '_scraper.py'
    })
    return response.data
  }
}
